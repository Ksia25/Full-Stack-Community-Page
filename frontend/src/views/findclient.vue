    ...
    const handleSubmitForm = async () => {
      if (searchBy.value === 'Client Name') {
        if (firstName.value || lastName.value) {
          try {
            const query = { searchBy: 'name', firstName: firstName.value, lastName: lastName.value };
            clients.value = await searchClients(query);
          } catch (error) {
        ...
        if (phoneNumber.value) {
          try {
            const query = { searchBy: 'number',phoneNumber:  phoneNumber.value };
            clients.value = await searchClients(query);
          } catch (error) {
            toast.error("Error searching clients: " + error.message);
          }
